...
namespace ESP.Standard.Data.PostgreSql
{
    ...
    public class SafeProcedure
    {
        ...
        public static int ExecuteNonQuery(Database database, string procedureName, ParameterMapper parameterMapper, OutputParameterMapper outputMapper)
        {
            ...
            try
            {
                using (NpgsqlConnection connection = database.GetOpenConnection())
                {
        ...
        /// 	<para>The <see cref="NpgsqlConnection"/> on which to execute sproc.
        ...
        /// 	<para>An unexpected exception has been encountered during the sproc call.</para>
        /// </exception>
        public static int ExecuteNonQuery(Database database, NpgsqlConnection connection, string procedureName,
        ...
        public static int ExecuteNonQuery<T>(Database database, string procedureName, ParameterMapper<T> parameterMapper, T objectInstance)
        {
            ...
            try
            {
                ...
                {
                    NpgsqlCommand command = CommandFactory.CreateParameterMappedCommand<T>(connection, database.InstanceName, procedureName, parameterMapper, objectInstance);
        ...
        public static int ExecuteNonQuery(Database database, string procedureName, params object[] parameterValues)
        {
            ...
            try
            {
                using (NpgsqlConnection connection = database.GetConnection())
                {
                    NpgsqlCommand command = CommandFactory.CreateCommand(connection, database.InstanceName, procedureName, parameterValues);
        ...
        /// 	<para>An unexpected exception has been encountered during the sproc call.</para>
        /// </exception>
        public static object ExecuteScalar(Database database, NpgsqlConnection connection, string procedureName, ParameterMapper parameterMapper, OutputParameterMapper outputMapper)
        {
            if (database == null) throw new ArgumentNullException("database");
            if (connection == null) throw new ArgumentNullException("connection");
            if (procedureName == null) throw new ArgumentNullException("procedureName");
        ...
        public static DataTable Execute(Database database, string procedureName, params object[] parameters)
        {
            ...
            catch (Exceptions.DataException ex) // Procedure class already wrapped all necessary data
            {
                throw ex;
            }
            catch (Exception e)
        ...
        public static void ExecuteAndMapResults(Database database, string procedureName, StoredProcedureParameterList parameterList, ResultMapper result, OutputParameterMapper outputMapper)
        {
            try
            {
                using (NpgsqlConnection connection = database.GetConnection())
                {
                    NpgsqlCommand command = CommandFactory.CreateCommand(connection, database.InstanceName, procedureName, parameterList);
        ...
        /// <param name="parameters"></param>
        /// <returns></returns>
        public static List<T> ExecuteAndGetInstanceList<T>(Database database, string procedureName, RecordMapper<T> recordMapper, NpgsqlParameter[] parameters) where T : new()
        {
            List<T> instanceList = new List<T>();
            ExecuteAndHydrateInstanceList(instanceList, database, procedureName, recordMapper, parameters);
            return instanceList;
        ...
        public static List<T> ExecuteAndGetInstanceList<T>(Database database, string procedureName,int page,int pagesize, RecordMapper<T> recordMapper, NpgsqlParameter[] parameters) where T : new()
        {
            List<T> instanceList = new List<T>();
        ...
        public static void ExecuteAndHydrateInstanceList<T>(List<T> instanceList, Database database, string procedureName, RecordMapper<T> recordMapper, NpgsqlParameter[] parameters) where T : new()
        {
            try
            {
                using (IRecordSet reader = Procedure.Execute(database, procedureName, parameters))
        ...
        public static void ExecuteAndHydrateInstanceList<T>(List<T> instanceList, Database database, string procedureName, int page, int pagesize, RecordMapper<T> recordMapper, NpgsqlParameter[] parameters) where T : new()
        {
            try
            {
